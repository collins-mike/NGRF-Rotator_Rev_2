...
def PatchEHPlanePlot(Freq, W, L, h, Er, isLog=True):
    #"""
    #Plot 2D plots showing E-field for E-plane (phi = 0degrees) and the H-plane (phi = 90degrees).
    #"""
    ...
    if isLog:                                                                                                   # Can plot the log scale or normal
        plt.plot(Xtheta, 20 * np.log10(abs(fields[90, :])), label="H-plane (Phi=90D)")                          # Log = 20 * log10(E-field)
        plt.plot(Xtheta, 20 * np.log10(abs(fields[0, :])), label="E-plane (Phi=0D)")
        plt.ylabel('E-Field (dB)')
    else:
        plt.plot(Xtheta, fields[90, :], label="H-plane (Phi=90D)")
        plt.plot(Xtheta, fields[0, :], label="E-plane (Phi=0D)")
        plt.ylabel('E-Field')
